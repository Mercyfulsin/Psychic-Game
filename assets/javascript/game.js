//GLOBAL VARIABLES
var guesses, wins, losses, remainder, wordList, answer, alphabetArr, underscoreArr, letterArr;
...
//HTML Variables
var hWins, hLosses, hRemainder, hGuesses, hAnswer, hWordArea;
...
document.onkeyup = function (event) {
    ...
    if (alphabetArr.indexOf(letter) === -1){
        alert("Please only press alphabet characters.");
...
function generateHTML(){
    ...
    var spacebar = document.createElement('span');
    spacebar.setAttribute("id","spacebar");
    spacebar.innerHTML = " ";
    for(var i = 0; i < answerArr.length; i++){
        if(alphabetArr.indexOf(answerArr[i]) === -1){
            console.log("adding space bar");
            hWordArea.appendChild(spacebar);
        }else{
            console.log("adding underscore");
            hWordArea.appendChild(underscore);
        }
    }
}